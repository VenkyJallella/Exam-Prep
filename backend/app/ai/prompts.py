...
2. **Content Must Be Current ({current_year})**:
   - All facts, data, statistics MUST be up-to-date as of {current_year}
   - Reference the LATEST exam patterns, syllabus changes, cut-offs from {current_year}
   - Include recent government policies, budget announcements, or reforms if relevant
   - Mention current year exam dates, application deadlines, and recent notifications
...
5. **Content Format**: Use Markdown formatting (## for H2, ### for H3, **bold**, - for bullets)

Return valid JSON:
{{
  "title": "SEO-optimised title including {current_year} (50-60 characters ideally)",
