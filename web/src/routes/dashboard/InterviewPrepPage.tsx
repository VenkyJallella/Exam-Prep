    ...
    <>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <div className="flex items-center gap-1 rounded-full bg-primary-50 px-3 py-1 text-sm font-medium text-primary-700 dark:bg-primary-900/20 dark:text-primary-400">
          ...
          <div className="flex items-center gap-1 rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700 dark:bg-green-900/20 dark:text-green-400">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        <select value={selectedDiff} onChange={e => { setSelectedDiff(e.target.value); setPage(1); }} className="input w-auto">
      ...
      {topics.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          <button onClick={() => { setSelectedTopic(''); setPage(1); }} className={`rounded-full px-3 py-1 text-sm font-medium transition-colors ${!selectedTopic ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400'}`}>All</button>
          {topics.map(t => (
            <button key={t.topic} onClick={() => { setSelectedTopic(t.topic); setPage(1); }} className={`rounded-full px-3 py-1 text-sm font-medium transition-colors ${selectedTopic === t.topic ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400'}`}>
            ...
            <div key={q.id} className={`overflow-hidden rounded-xl border transition-shadow ${isExpanded ? 'border-primary-300 shadow-lg dark:border-primary-700' : 'border-gray-200 hover:shadow-md dark:border-gray-800'} bg-white dark:bg-gray-900`}>
              ...
              <div className="flex items-start gap-3 p-5">
                ...
                <div className="flex shrink-0 items-center gap-2 pt-1">
                  <button onClick={() => toggleBookmark(q.id)} title={q.is_bookmarked ? 'Remove bookmark' : 'Bookmark'}>
                  ...
                  <label className="flex cursor-pointer items-center gap-1" title="Mark as practiced">
                    <input type="checkbox" checked={q.is_practiced} onChange={() => togglePracticed(q.id)} className="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500" />
                  </label>
                  <svg onClick={() => setExpandedId(isExpanded ? null : q.id)} className={`h-5 w-5 cursor-pointer text-gray-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                </div>
              </div>
